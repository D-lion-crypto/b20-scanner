...
本项目默认是只读扫描工具，不读取钱包私钥，也不会在后端自动签名交易。页面里的导入、买入、卖出、部署提交都会交给浏览器钱包确认。
...
- 支持一键请求钱包导入代币。
- 支持检测到 `B20 Asset` 上线后自动弹出钱包部署交易确认。
- 支持导出 CSV / JSON / HTML。
...
如果你已经在钱包或前端里准备好了 `launchB20` 交易，但因为 `FeatureNotActivated(bytes32)` 失败，可以把同一笔交易的参数放到网页里等待上线后自动提交。

使用步骤：

1. 填入 `launchB20 合约地址`。
2. 填入 `launchB20 calldata`，必须是 `0x` 开头的完整十六进制交易数据。
3. 填入交易 `value`，单位是 ETH，例如 `0.01`。
4. 点击 `连接钱包`，确认钱包网络是 Base。
5. 点击 `启用上线后自动提交`。
6. 页面每 3 秒检查一次 `B20 Asset`，一旦返回 `true`，会自动调用钱包弹出交易确认。

注意：

- 页面必须一直打开，电脑和浏览器不能休眠。
- 工具不会保存私钥，也不会绕过钱包确认。
- 如果钱包拒绝、余额不足、参数错误或 gas 失败，页面会停止自动提交，需要你检查后重新启用。
- 这个功能只负责在上线瞬间提交你准备好的交易，不负责判断代币参数、流动性或价格风险。
